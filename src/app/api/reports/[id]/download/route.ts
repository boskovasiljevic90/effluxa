...
export async function GET(req: NextRequest, { params }: Props) {
  try {
    ...
    const title = workspace.owner.companyName
      ? `${workspace.owner.companyName} Financial Audit`
      : "Effluxa AI Financial Leak Audit";

    text(title, 24, true, rgb(0.05, 0.1, 0.2));
    y -= 10;

    if (workspace.owner.companyName) {
    ...
    y -= 20;
    if (workspace.owner.reportFooter) {
      paragraph(workspace.owner.reportFooter);
      y -= 10;
    }
